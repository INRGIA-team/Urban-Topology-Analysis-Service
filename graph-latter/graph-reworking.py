...
def rebuilding_graph(inputfile, outputfile):
    ...
    renaming_ways(input_graph_root)
...
def attributes():
    ...
    attrib = [({'id': keys[0], "for": "node", "attr.name": "highway", "attr.type": "string"}, 'undefined'),
              ...
              ({'id': keys[3], "for": "node", "attr.name": "surface", "attr.type": "string"}, 'undefined'),
...
def renaming_ways(graph_root):
    for way in graph_root.iter("way"):
        for tag in way.iter("tag"):
            if tag.get("k") == "name" and tag.get("v") == "nan":
                for nd in way.iter("nd"):
                    street_name = API_request.get_name(graph_root, nd.get("ref"))
                    if street_name != None:
...
def main():
    rebuilding_graph(inputfile='MurinoLO.osm', outputfile='reversed-MurinoLO-4.graphml')
